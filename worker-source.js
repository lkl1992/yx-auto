...
// 生成VX链接
function generateVxLinksFromSource(list, user, workerDomain, disableNonTLS = false, customPath = '/') {
    const CF_HTTP_PORTS = [80, 8080, 8880, 2052, 2082, 2086, 2095];
    const CF_HTTPS_PORTS = [443, 2053, 2083, 2087, 2096, 8443];
    const defaultHttpsPorts = [443];
    const defaultHttpPorts = disableNonTLS ? [] : [80];
    ...
    list.forEach(item => {
        ...
        portsToGenerate.forEach(({ port, tls }) => {
            const vxConfig = {
                v: "2",
                ps: tls ? `${nodeNameBase}-${port}-VX-WS-TLS` : `${nodeNameBase}-${port}-VX-WS`,
                add: safeIP,
                port: port.toString(),
                id: user,
                aid: "0",
            ...
            if (tls) {
                vxConfig.sni = workerDomain;
                vxConfig.fp = "chrome";
            }
            const vxBase64 = btoa(JSON.stringify(vxConfig));
            const vmessProto = btoa('vmess://');
            links.push(`${vmessProto}${vxBase64}`);
        });
    });
    return links;
}
...
async function handleSubscriptionRequest(request, user, customDomain, piu, ipv4Enabled, ipv6Enabled, ispMobile, ispUnicom, ispTelecom, evEnabled, etEnabled, vmEnabled, disableNonTLS, customPath) {
    ...
    async function addNodesFromList(list) {
        ...
        if (vmEnabled) {
            finalLinks.push(...generateVxLinksFromSource(list, user, nodeDomain, disableNonTLS, wsPath));
        }
    }
            ...
            <div class="form-group" style="margin-top: 24px;">
                ...
                <div style="margin-top: 8px;">
                    ...
                    <div class="list-item" onclick="toggleSwitch('switchVM')">
                        <div>
                            <div class="list-item-label">VX (vm)</div>
                        </div>
    ...
    <script>
        ...
        function generateClientLink(clientType, clientName) {
            ...
            if (!switches.switchVL && !switches.switchTJ && !switches.switchVM) {
                alert('请至少选择一个协议（VLESS、Trojan或VX）');
                return;
            }
